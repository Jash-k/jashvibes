...
async function getTmdbMetaForStremio({ id = '', type = 'movie' } = {}) {
  ...
  try {
    if (type === 'series') {
      ...
      return {
        id: external?.imdb_id || id,
        imdb_id: external?.imdb_id || id,
        type: 'series',
...
export async function getStremioStreams({ type = 'movie', id = '', source = '' } = {}) {
  ...
  let rawStreams = payload.streams || [];
  let resolvedId = id;
  ...
  if (!streams.length && parseTmdbRef(id)) {
    const imdbId = await getImdbIdForStremio({ id, type: streamType });
    if (imdbId) {
      const imdbUrl = `${base}/stream/${encodeURIComponent(streamType)}/${encodeURIComponent(imdbId)}.json`;
      try {
        const imdbPayload = await fetchJson(imdbUrl, { timeoutMs: Number(process.env.STREMIO_STREAM_TIMEOUT_MS || DEFAULT_TIMEOUT_MS) });
        rawStreams = imdbPayload.streams || [];
        streams = rawStreams
          .filter((stream) => isSafeDirectStream(stream, { source }))
          .map(mapStremioStream)
          .sort((a, b) => b.qualityScore - a.qualityScore || b.sizeBytes - a.sizeBytes);
