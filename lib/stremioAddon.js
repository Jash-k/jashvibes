...
// Built-in Global Stremio addon fallback — used only when no STREMIO* env var
// is configured, so the app still streams out of the box. Env always wins.
import { stremioRegistryOverride } from '@/lib/stremioRegistry';

const DEFAULT_STREMIO_MANIFEST_URL = 'https://mobile-marika-magizchisoftware-b0475823.koyeb.app/stremio/thalaajith/manifest.json';
...
function configuredManifestUrl({ source = '', manifestUrl = '' } = {}) {
  ...
  // Admin registry wins when it has an enabled addon for this kind. The
  // 60-second process cache is warmed by the Stremio/admin entry routes
  // (lib/stremioRegistry.js) — empty registry = the env chain below applies.
  const registry = stremioRegistryOverride(source === 'watch' || source === 'provider' ? 'watch' : 'catalog');
  ...
  return generic || DEFAULT_STREMIO_MANIFEST_URL;
}
