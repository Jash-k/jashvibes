...
export function getStremioBaseUrl() {
  const manifestUrl = normalizeHttpUrl(configuredManifestUrl());
  if (!manifestUrl) return '';
  return manifestUrl.replace(/\/manifest\.json(?:\?.*)?$/i, '').replace(/\/+$/, '');
}

export function getStremioManifestUrl() {
...
async function fetchJson(url, { timeoutMs = DEFAULT_TIMEOUT_MS } = {}) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Stremio addon returned HTTP ${response.status}`);
    if (!payload) throw new Error('Stremio addon returned a non-JSON response');
    return payload;
  } catch (error) {
...
export async function getStremioManifest() {
  const url = getStremioManifestUrl();
...
export async function getStremioMeta({ type = 'movie', id = '' } = {}) {
  if (!id) throw new Error('Stremio id is required');
  const base = getStremioBaseUrl();
  if (!base) throw new Error('Stremio addon is not configured. Set STREMIO to your addon manifest URL.');
...
function allowedHost(url) {
  ...
  try {
    ...
    const baseHost = new URL(getStremioBaseUrl()).hostname.toLowerCase();
...
function isSafeDirectStream(stream = {}) {
...
export async function getStremioStreams({ type = 'movie', id = '' } = {}) {
