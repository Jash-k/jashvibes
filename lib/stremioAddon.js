...
export async function getStremioCatalog({ type = 'movie', catalogId = '', skip = 0, search = '', source = 'catalog' } = {}) {
  ...
  const cacheKey = `${base}|${source}|${type}|${selectedCatalogId}|${skip}|${search}`;
...
function buildStreamMetaLabel(stream = {}, index = 0) {
  ...
  const parts = uniqueMetaParts([resolution, ...audio]);
...
export function mapStremioStream(stream = {}, index = 0) {
  ...
  return {
    ...
    label: buildStreamMetaLabel(stream, index),
...
export async function getStremioStreams({ type = 'movie', id = '', source = '', season = 1, episode = 1 } = {}) {
  ...
  const isEpisodeStyleId = /^tt\d+:\d+:\d+$/i.test(String(id || ''));
  ...
  const cacheKey = `${base}|${source}|${streamType}|${id}|s${wantedSeason}|e${wantedEpisode}`;
  ...
  if (streamType === 'series') {
    if (/^tt\d+$/i.test(requestId)) requestId = `${requestId}:${wantedSeason}:${wantedEpisode}`;
    else if (parseTmdbRef(requestId)) {
      const imdbId = await getImdbIdForStremio({ id: requestId, type: 'series' });
      if (imdbId) {
        requestId = `${imdbId}:${wantedSeason}:${wantedEpisode}`;
        fallbackResolved = true;
      }
    }
  } else if (parseTmdbRef(requestId)) {
    const imdbId = await getImdbIdForStremio({ id: requestId, type: 'movie' });
    if (imdbId) {
      requestId = imdbId;
      fallbackResolved = true;
    }
  ...
  const url = `${base}/stream/${encodeURIComponent(streamType)}/${encodeURIComponent(requestId)}.json`;
