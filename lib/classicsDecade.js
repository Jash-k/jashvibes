 ...
 *
 * Only decades that actually hold titles are returned. Filling the span with zeros looked like honesty
 * on paper and read as a broken control on screen — a `1940s` tab you cannot press — so an absence is
 * simply not offered. Completeness is still provable, because `dated + undated` has to equal what the
 * filtered shelf holds, and that check is what `unaccounted` reports.
 *
