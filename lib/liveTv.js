...
export const DEFAULT_SOURCES = [
  ...
  {
    // Romaxa55's world_ip_tv: every channel is re-verified NIGHTLY by a GitHub
    // Actions bot that actually fetches the streams (not just pings manifests)
    // and drops dead entries. Verified end-to-end 2026-09-25: manifests chain
    // 2-3 hops down to real multi-MB TS segments. titleFilter trims the 14k
    // entries to sports; the segment-level health check + hourly auto-sync
    // flag anything that rots between their nightly rebuilds. Lesson from
    // v10.2.0: a 200-OK manifest proves nothing — the cricket packs shipped
    // 200 manifests over 404 segments ("SRC_NOT_SUPPORTED" for the user).
    id: 'romaxa55-sports',
    label: 'Sports (nightly verified)',
    type: 'm3u',
    url: 'https://romaxa55.github.io/world_ip_tv/output/index.m3u',
    trustTamil: false,
    priority: 5,
    autoPurge: true,
    titleFilter: 'star sports|sony sports|sony ten|sports ten|dd sports|sports ?18|fancode|fan code|willow|cricket|criclife|crichd|cricmagic|ptv sports|ten sports|\\bt sports\\b|\\ba sports\\b',
  },
];
...
async function verifyStreamDeliversVideo(url, { signal } = {}) {
  let current = { url, text: '' };
  for (let hop = 0; hop < 3; hop += 1) {
    if (!current.text) {
      const res = await fetch(current.url, {
        headers: buildPlaybackHeaders({}, current.url),
        redirect: 'follow',
        cache: 'no-store',
        signal,
      });
    ...
    if (lines.some((line) => line.startsWith('#EXTINF'))) {
      ...
      const segmentRes = await fetch(segmentUrl, {
        headers: { ...buildPlaybackHeaders({}, segmentUrl), Range: 'bytes=0-262143' },
        redirect: 'follow',
        cache: 'no-store',
        signal,
      });
  ...
  try {
    ...
    if (channel.format === 'dash' || channel.format === 'hls') {
      ...
      if (ok && channel.format === 'hls' && !isJioChannel(channel)) {
        try {
          const proof = await verifyStreamDeliversVideo(uri, { signal: controller.signal });
          if (!proof.ok || proof.codec === 'hevc') ok = false;
        } catch {
          ok = false;
        }
      }
