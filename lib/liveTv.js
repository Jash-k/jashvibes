const DEFAULT_SOURCES = [
  {
    ...
    label: 'Zoh Tamil',
    type: 'm3u',
    url: 'https://raw.githubusercontent.com/joiptv/Jo/refs/heads/main/Zoh.txt',
    trustTamil: false,
    priority: 0,
  },
  {
  ...
  {
    id: 'jio-auto-tamil',
    label: 'Jio Auto Tamil',
    type: 'm3u',
    url: 'https://raw.githubusercontent.com/sportlive18/Jio-Auto-Update-m3u-playlist/refs/heads/main/jtv.m3u',
    trustTamil: false,
    priority: 2,
  },
  {
...
const WORKING_CHECK_TIMEOUT_MS = Number(process.env.LIVE_WORKING_TIMEOUT_MS || 4500);
const WORKING_CHECK_CONCURRENCY = Number(process.env.LIVE_WORKING_CONCURRENCY || 18);
...
function parsePipedStreamUrl(value = '') {
  ...
  return {
    ...
    cookie: options.cookie || options.cookies || '',
...
function mapChannel(raw, source, index = 0) {
  const piped = parsePipedStreamUrl(raw.url || raw.streamUrl || raw.link || raw.href || '');
  ...
  return {
    ...
    cookie: raw.cookie || raw.cookies || piped.cookie || '',
    userAgent: raw.userAgent || raw.user_agent || raw.ua || piped.userAgent || '',
    referer: raw.referer || raw.referrer || raw.origin || piped.referer || '',
...
function getSources() {
  ...
  if (custom.length && process.env.LIVE_TV_REPLACE_DEFAULTS === '1') return custom;
...
function buildPlaybackHeaders(channel = {}, uri = '') {
  ...
  const jioLike = text.includes('jio') || text.includes('jiotv');
...
export async function getLiveTVChannels({ source = 'all', playableOnly = false, workingOnly = false } = {}) {
  ...
  const workingChannels = workingOnly ? await filterWorkingChannels(injectedChannels) : injectedChannels;
  const channels = dedupeChannels(workingChannels);
