 ...
 *
 * `extraSupported` entries may be bare names or `{ name, value: { options } }` — and when the addon spells
 * out its options, those ARE the filter: the values it will accept and the words it labels them with.
 * Reading them here is what lets the shelf stop guessing (the old page hardcoded `Tamil`, `Crime`,
 * `Highest Rated` and sent them whether or not the addon used those strings).
 */
export function normalizeCatalog(catalog = {}) {
  const raw = Array.isArray(catalog.extraSupported) && catalog.extraSupported.length
    ? catalog.extraSupported
    : (Array.isArray(catalog.extra) ? catalog.extra : []);
  const names = [];
  const extraOptions = {};
  for (const entry of raw) {
    const name = String(typeof entry === 'string' ? entry : (entry?.name || '')).toLowerCase();
    if (!name) continue;
    names.push(name);
    const declared = entry?.value?.options ?? entry?.options;
    if (!Array.isArray(declared) || !declared.length) continue;
    extraOptions[name] = declared
  ...
  return {
    ...
    extraSupported: names,
    extraOptions,
  };
}

/**
 * The options to offer for one filter on one catalog: the addon's own list when it declared one, so the
 * value sent is a value it recognises; the app's list otherwise.
 */
export function filterOptions(catalog = {}, fieldId = '', fallback = []) {
  for (const alias of EXTRA_ALIASES[fieldId] || [fieldId]) {
    const declared = catalog?.extraOptions?.[alias];
    if (Array.isArray(declared) && declared.length) return declared;
  }
  return fallback;
}

/** What a chip should read for a value the addon named differently from the label it sent. */
export function optionLabel(catalog = {}, fieldId = '', value = '', fallback = []) {
  if (!value) return '';
  const hit = filterOptions(catalog, fieldId, fallback).find((option) => option.value === value);
