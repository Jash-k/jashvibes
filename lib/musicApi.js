...
const HOME_SECTIONS = [
  { id: 'trending-tamil', title: 'Trending Tamil', query: 'Tamil 2026', limit: 18 },
  { id: 'anirudh-hits', title: 'Anirudh Hits', query: 'Anirudh Ravichander Tamil', limit: 18 },
  { id: 'arr-essentials', title: 'ARR Essentials', query: 'A.R. Rahman Tamil', limit: 18 },
  { id: 'ilaiyaraaja-classics', title: 'Ilaiyaraaja Classics', query: 'Ilaiyaraaja Tamil', limit: 18 },
  { id: 'yuvan-hits', title: 'Yuvan Hits', query: 'Yuvan Shankar Raja Tamil', limit: 18 },
  { id: 'harris-hits', title: 'Harris Jayaraj Hits', query: 'Harris Jayaraj Tamil', limit: 18 },
  { id: 'tamil-love', title: 'Tamil Love', query: 'Tamil love songs', limit: 18 },
  { id: 'tamil-90s', title: 'Tamil 90s', query: 'Tamil 1990s', limit: 18 },
];
...
function streamsFromSaavn(item = {}) {
  const list = item.downloadUrl || item.download_url || [];
...
export function mapSaavnTrack(item = {}) {
  ...
  return {
    ...
    artistList,
    album: normalizeWhitespace(item.album?.name || item.album || item.album_title || ''),
    albumSeokey: item.album?.id || item.album_id || '',
...
export async function getTrending(_language = 'Tamil', limit = 24) {
  const buckets = await Promise.all(
    HOME_SECTIONS.slice(0, 5).map((section) => fetchSaavn('/api/search/songs', { query: section.query, limit: 10 }).catch(() => null)),
  );
  const items = buckets
    .flatMap((payload) => firstArray(payload).map(mapSaavnTrack))
    .filter((track) => track.title)
    .filter((track) => String(track.language || '').toLowerCase() === 'tamil');
  return dedupeTracks(sortTamilFirst(items)).slice(0, limit);
}
...
export async function getTamilArtists() {
  ...
  return results.map((payload, index) => {
    ...
    const item = firstArray(payload)[0] || { name: targetName };
...
export async function getMusicHome() {
  ...
  const [sections, artists, playlists, releases] = await Promise.all([
    Promise.all(HOME_SECTIONS.map(async (section) => {
      try {
        return { ...section, type: 'tracks', items: await searchSongs(section.query, section.limit) };
      } catch (error) {
        warnings.push(`${section.title}: ${error.message}`);
        return { ...section, type: 'tracks', items: [], warning: error.message };
      }
    })),
    getTamilArtists().catch((error) => { warnings.push(`Artists: ${error.message}`); return []; }),
  ...
  return { sections, artists, playlists, releases, warning: warnings[0] || '', warnings };
}

export async function getSongInfo(seokey) {
  if (!String(seokey || '').startsWith('saavn:')) throw new Error('Only JioSaavn songs are supported');
  const id = String(seokey).replace(/^saavn:/, '');
