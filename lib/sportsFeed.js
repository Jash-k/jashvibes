...
export async function loadM3uSource({ kind = 'fancode', url = '', fetchImpl = fetch, env = process.env, now = Date.now() } = {}) {
  ...
  try {
    ...
    return {
      ...
      via: 'm3u',
      note: stale
        ? `playlist dated ${parsed.dumpAt || 'unknown'} · too old to show`
        : (parsed.dumpAt ? `playlist dated ${parsed.dumpAt}` : 'playlist carries no date'),
      ageMs: parsed.ageMs,
      stale,
      rows: parsed.entries.map((entry) => ({ ...entry, __kind: kind, __dumpAt: parsed.dumpAt, __dumpStale: stale })),
    };
  } catch (error) {
 ...
 * The playlist family is primary; the legacy FanCode JSON dump is fetched only when the playlist failed —
 * a 14-day-old snapshot is not worth a request while the 15-minute playlist is answering.
 */
export async function loadFeed({ fetchImpl = fetch, env = process.env, now = Date.now() } = {}) {
  const [bcciLive, bcciSoon, bcciDone, wt20, fanM3u, sonyM3u, iccM3u] = await Promise.allSettled([
  ...
  const slot = (result) => (result.status === 'fulfilled' ? result.value : { ok: false, rows: [], error: result.reason?.message || 'failed' });
  const fanM3uValue = slot(fanM3u);
  /* The legacy hand-published dump: same matches, JSON shape. Only asked for when the playlist
     could not answer, so the board never pays for a snapshot it does not need. */
  const fancodeSlot = fanM3uValue.ok
    ? { status: 'fulfilled', value: { ok: false, rows: [], via: '', note: 'skipped · the live playlist answered', error: '' } }
    : { status: 'fulfilled', value: await loadFancodeFeed({ fetchImpl, env, now }) };
  const settled = (result, normalizer) => {
    const value = slot(result);
    return {
      ok: Boolean(value.ok),
      items: (value.rows || []).map((row) => normalizer(row, { now })).filter(Boolean),
      error: value.error || '',
      via: value.via || '',
      note: value.note || '',
  ...
  const batches = [
    ...
    settled(fanM3u, (row, opts) => normalizeM3uEntry(row, { ...opts, kind: 'fancode', dumpAt: row.__dumpAt, stale: row.__dumpStale })),
    settled(sonyM3u, (row, opts) => normalizeM3uEntry(row, { ...opts, kind: 'sonyliv', dumpAt: row.__dumpAt, stale: row.__dumpStale })),
    settled(iccM3u, (row, opts) => normalizeM3uEntry(row, { ...opts, kind: 'icc', dumpAt: row.__dumpAt, stale: row.__dumpStale })),
    settled(fancodeSlot, normalizeFancode),
  ];
  const merged = mergeFeed({ batches, now });
  const liveCount = merged.items.filter((item) => item.state === 'live').length;
  const labels = ['bcci-live', 'bcci-upcoming', 'bcci-recent', 'icc-wt20', 'fancode-m3u', 'sonyliv-m3u', 'icc-m3u', 'fancode-legacy'];
  return {
