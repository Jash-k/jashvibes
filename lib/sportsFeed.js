...
/* The live playlist family (round 23): the JSON dump above went quiet on 2026-09-03 while the same
   publisher's `.m3u` playlists kept refreshing every ~15 minutes, and the family grew a SonyLiv and an
   ICC sibling. The parser is `lib/sportsM3u.js`; these are the defaults, each env-overridable. */
import { M3U_FANCODE, M3U_SONYLIV, M3U_ICC, parseM3u } from './sportsM3u.js';
...
/* A hand-published dump older than this is stale: its "LIVE" rows describe matches that ended long ago.
   6 h with the IST assumption below means a fresh dump is never misjudged over a zone ambiguity. */
export const STALE_DUMP_MS = 6 * 60 * 60_000;

export function parseDumpDate(raw = '') {
  const match = String(raw || '').match(/(\d{1,2}):(\d{2})(?::(\d{2}))?\s*([AP])\.?M\.?\s+(\d{1,2})-(\d{1,2})-(\d{4})/i);
  if (!match) {
    const fallback = Date.parse(String(raw || ''));
    return Number.isFinite(fallback) ? fallback : 0;
  }
  const [, hh, mm, ss, ap, dd, mon, yyyy] = match;
  const hour24 = (Number(hh) % 12) + (/p/i.test(ap) ? 12 : 0);
  return Date.UTC(Number(yyyy), Number(mon) - 1, Number(dd), hour24 - 5, Number(mm) - 30, Number(ss) || 0);
}
...
async function loadIccJson({ path, params, fetchImpl = fetch, env = process.env }) {
  ...
  for (const url of [`${ICC_API}/${path}?${q}`, `${sportsBackend(env)}/api/wt20/${path}?${q}`]) {
    try {
      return await getJson(url, { fetchImpl, headers: ICC_HEADERS, timeoutMs: 8_000 });
    } catch (error) {
...
function makeMatch(input = {}) {
  ...
  return {
    ...
    staleFeed: Boolean(input.staleFeed),
...
export function normalizeFancode(row = {}, { now } = {}) {
  ...
  return makeMatch({
    ...
    staleFeed: Boolean(row.__dumpStale),
...
export async function loadBcciFeed({ feed = 'live', fetchImpl = fetch, env = process.env } = {}) {
  ...
  const attempts = [
    { label: 'movies1', run: () => getJson(`${root}/api/bcci/${feed}`, { fetchImpl, timeoutMs: 8_000 }) },
    { label: 'bcci', run: () => getJson(bcciScoreUrl(feed), { fetchImpl, headers: BCCI_HEADERS, text: true, timeoutMs: 8_000 }).then((body) => JSON.parse(body)) },
  ];
  const errors = [];
...
export async function loadWt20Feed({ fetchImpl = fetch, env = process.env } = {}) {
  try {
    const payload = await getJson(`${sportsBackend(env)}/api/wt20/schedule`, { fetchImpl, timeoutMs: 8_000 });
...
export async function loadFancodeFeed({ fetchImpl = fetch, env = process.env, now = Date.now() } = {}) {
  try {
    ...
    const dumpMs = parseDumpDate(dumpAt);
    const ageMs = dumpMs ? Math.max(0, now - dumpMs) : 0;
    // Undated dumps cannot be judged, so they are not flagged — only provably old ones are.
    const stale = dumpMs ? ageMs > STALE_DUMP_MS : false;
    return {
      ok: true,
      via: 'dump',
      ...
      ageMs,
      stale,
      rows: rows.map((row) => ({ ...row, __headers: headers, __dumpAt: dumpAt, __dumpStale: stale })),
    };
  } catch (error) {
...
/**
 * The live playlist family — the primary FanCode source since round 23, plus SonyLiv and ICC.
 * Each playlist is text, fetched like any other source and parsed by `lib/sportsM3u.js`. Its
 * `#DATE:` line is the dump clock here: older than STALE_DUMP_MS means the playlist is not a
 * claim about today, so its rows are dropped entirely (an expired playlist has nothing honest
 * left to say — the ICC one, for example, only refreshes while an event is on).
 */
export async function loadM3uSource({ kind = 'fancode', url = '', fetchImpl = fetch, env = process.env, now = Date.now() } = {}) {
  const resolved = (kind === 'fancode' && (env.SPORTS_FANCODE_M3U_URL || undefined))
    || (kind === 'sonyliv' && (env.SPORTS_SONYLIV_M3U_URL || undefined))
    || (kind === 'icc' && (env.SPORTS_ICC_M3U_URL || undefined))
    || { fancode: M3U_FANCODE, sonyliv: M3U_SONYLIV, icc: M3U_ICC }[kind];
  try {
    const text = await getJson(`${resolved}?_=${Date.now()}`, { fetchImpl, text: true });
    const parsed = parseM3u(text, { now });
    if (!parsed.entries.length) return { ok: false, rows: [], error: `${kind}: playlist carried no entries` };
    // Undated playlists cannot be judged, so they are not flagged — only provably old ones are dropped.
    const stale = parsed.dumpMs ? parsed.ageMs > STALE_DUMP_MS : false;
    if (stale) {
      return {
        ok: false, via: 'm3u', stale: true, ageMs: parsed.ageMs,
        note: `playlist dated ${parsed.dumpAt || 'unknown'} · too old to show`,
        rows: parsed.entries.map((entry) => ({ ...entry, __kind: kind, __dumpAt: parsed.dumpAt, __dumpStale: true })),
      };
    }
    /* Only working streams reach the board: each entry's manifest is asked for right now,
       with the entry's own headers, and anything that does not answer is hidden — the
       playlist's words are not enough, the stream has to be up. */
    const verdicts = await Promise.allSettled(parsed.entries.map((entry) => verifyM3uEntry(entry, fetchImpl)));
    const alive = [];
    let dead = 0;
    verdicts.forEach((verdict, index) => {
      if (verdict.status === 'fulfilled' && verdict.value) alive.push(parsed.entries[index]);
      else dead += 1;
    });
    return {
      ok: alive.length > 0,
      via: 'm3u',
      note: [
        parsed.dumpAt ? `playlist dated ${parsed.dumpAt}` : 'playlist carries no date',
        dead ? `${dead} entr${dead === 1 ? 'y' : 'ies'} not answering (hidden)` : '',
      ].filter(Boolean).join(' · '),
      ageMs: parsed.ageMs,
      stale: false,
      rows: alive.map((entry) => ({ ...entry, __kind: kind, __dumpAt: parsed.dumpAt, __dumpStale: false })),
    };
  } catch (error) {
    return { ok: false, rows: [], error: `${kind}: ${error.message}` };
  }
}

/** A stream is working when its master manifest answers, right now, with a manifest. */
async function verifyM3uEntry(entry = {}, fetchImpl = fetch) {
  const url = String(entry.url || '');
  if (!/^https?:\/\//i.test(url)) return false;
  try {
    const headers = {};
    if (entry.userAgent) headers['User-Agent'] = entry.userAgent;
    if (entry.referer) headers.Referer = entry.referer;
    if (entry.origin) headers.Origin = entry.origin;
    const body = await getJson(`${url}${url.includes('?') ? '&' : '?'}_=${Date.now()}`, { fetchImpl, headers, text: true, timeoutMs: 5_000 });
    return /#EXTM3U|#EXT-X|<\?xml|:MPD/i.test(String(body || '').slice(0, 400));
  } catch {
    return false;
  }
}

const M3U_SOURCE_LABEL = { fancode: 'FanCode', sonyliv: 'SonyLiv', icc: 'ICC' };

/** One playlist entry → the ONE board shape. The playlist says "on air now"; staleness is decided above it. */
export function normalizeM3uEntry(entry = {}, { kind = 'fancode', now = Date.now(), dumpAt = '', stale = false } = {}) {
  if (stale) return null;
  const name = FIRST(entry.name, entry.tvgName);
  if (!name) return null;
  const split = name.split(/\s+v[s_]?\.?\s+/i);
  const home = FIRST(split[0]) || name;
  const away = FIRST(split[1]) || '';
  const label = M3U_SOURCE_LABEL[kind] || kind.toUpperCase();
  const variant = {
    id: `${entry.id || kind}-${entry.langCode || entry.lang || 'en'}`.slice(0, 64),
    label: FIRST(entry.lang, entry.langCode) || `${label} live`,
    url: entry.url,
    expiresAt: '',
    cookie: '',
    ...
    origin: entry.origin || '',
    keyId: entry.clearKeyId || '',
    key: entry.clearKey || '',
    licenseKey: entry.clearKeyId && entry.clearKey ? `${entry.clearKeyId}:${entry.clearKey}` : '',
    format: entry.format || 'hls',
  };
  return makeMatch({
    id: entry.id || name,
    source: kind,
    sport: /cricket/i.test(entry.group) ? 'cricket' : (FIRST(entry.group).toLowerCase() || 'other'),
    competition: FIRST(entry.group, entry.tvgName, label),
    home,
    away,
    statusLine: `on air on ${label}${dumpAt ? ` · playlist published ${dumpAt}` : ''}`,
    statusRaw: 'live',
    state: 'live',
    timeKnown: false,
    startAt: null,
    startLabel: dumpAt ? `playlist ${dumpAt}` : '',
    poster: entry.poster || '',
    dumpAt,
    staleFeed: false,
    links: kind === 'fancode' && /^\d+$/.test(entry.id || '')
      ? { match: `https://www.fancode.com/cricket/x/tour/x/matches/x-${ID(entry.id)}/live-match-info` }
      : {},
    stream: entry.url,
    variants: [variant],
  });
...
/**
 * The whole Sports feed in one call: the live playlist family, merged, with per-source honesty.
 * Deliberately nothing else — no score backends, no fixtures, no results. `/sports` is a set of
 * working live streams, not a scoreboard; the playlists are read and verified in one request
 * window and everything the board shows was answering seconds ago.
 */
export async function loadFeed({ fetchImpl = fetch, env = process.env, now = Date.now() } = {}) {
  const [fanM3u, sonyM3u, iccM3u] = await Promise.allSettled([
    loadM3uSource({ kind: 'fancode', fetchImpl, env, now }),
    loadM3uSource({ kind: 'sonyliv', fetchImpl, env, now }),
    loadM3uSource({ kind: 'icc', fetchImpl, env, now }),
  ]);
  const settled = (result, kind) => {
    const value = result.status === 'fulfilled' ? result.value : { ok: false, rows: [], error: result.reason?.message || 'failed' };
    return {
      ok: Boolean(value.ok),
      items: (value.rows || []).map((row) => normalizeM3uEntry(row, { kind, now, dumpAt: row.__dumpAt, stale: row.__dumpStale })).filter(Boolean),
      error: value.error || '',
      via: value.via || '',
      note: value.note || '',
      ageMs: Number(value.ageMs) || 0,
      stale: Boolean(value.stale),
    };
  };
  const batches = [
    settled(fanM3u, 'fancode'),
    settled(sonyM3u, 'sonyliv'),
    settled(iccM3u, 'icc'),
  ];
  ...
  return {
    ...
    sources: batches.map((batch, index) => ({
      id: ['fancode-m3u', 'sonyliv-m3u', 'icc-m3u'][index],
      ok: batch.ok,
      count: batch.items.length,
      at: batch.items.find((item) => item.dumpAt)?.dumpAt || '',
      ageMs: batch.ageMs || 0,
      stale: Boolean(batch.stale),
      note: batch.ok ? [batch.via && `via ${batch.via}`, batch.note].filter(Boolean).join(' · ') : `unavailable · ${batch.note || batch.error}`,
    })),
  };
}
  ...
  const run = (async () => {
    ...
    try {
      ...
      const payload = {
        ...
        unavailable: true,
        items: [],
        counts: { live: 0, soon: 0, done: 0, tbc: 0, unverified: 0 },
        sources: [],
...
export function mergeFeed({ batches = [] } = {}) {
  const order = { bcci: 0, ipl: 1, icc: 2, fancode: 3, sonyliv: 4 };
  const byKey = new Map();
  for (const batch of batches) {
    for (const item of batch.items || []) {
      ...
      // A stale feed loses every tie: a days-old dump must never outrank a fresh BCCI/ICC row.
      // Fresh-vs-fresh ties behave exactly as before (both penalties are 0).
      const orderOf = (entry) => (order[entry.source] ?? 9) + (entry.staleFeed ? 10 : 0);
      const betterSource = orderOf(current) - orderOf(item);
  ...
  items.sort((a, b) => ((a.staleFeed ? 1 : 0) - (b.staleFeed ? 1 : 0)) || rank(a) - rank(b) || dayValue(b) - dayValue(a) || text(a.competition).localeCompare(text(b.competition)));
...
/* A stale feed's "live" rows are quarantined out of the live count: the board, the pill and the
   auto-poll trigger all read `counts.live`, so a 14-day-old dump can no longer shout LIVE. */
const countStates = (items) => items.reduce((acc, item) => {
  const key = item.state === 'live' && item.staleFeed ? 'unverified' : item.state;
  return { ...acc, [key]: (acc[key] || 0) + 1 };
}, { live: 0, soon: 0, done: 0, tbc: 0, unverified: 0 });
...
export async function loadHub({ source = 'bcci', id = '', seed = {}, fetchImpl = fetch, env = process.env, now = Date.now() } = {}) {
  ...
  } else if (source === 'sonyliv') {
    /* A SonyLiv playlist entry is a live channel, not a scored match: there is no scorecard
       endpoint to ask, so the hub says so and lets the common code put the stream in Video. */
    panels.scorecard = { state: 'empty', source: 'SonyLiv', note: 'this is a live channel stream · it carries no scorecard feed' };
    panels.commentary = { state: 'empty', source: 'SonyLiv', note: 'no ball-by-ball feed exists for a channel stream' };
  } else {
