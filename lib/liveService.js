...
// The plain-HLS sports sources auto-map their whole sports group into the
// sports catalog at onboarding (the admin prunes, instead of hand-mapping
// hundreds of channels). Tokened sources keep the strict cricket-only rule.
const SPORTS_CATALOG_SOURCE_IDS = new Set([]); // v10.3.0: no source auto-maps its whole sports group; cricket-only rule applies
...
async function autoMapNewSourceCricketChannels(sourceId) {
  ...
  const candidates = [...cricketDocs];
  if (SPORTS_CATALOG_SOURCE_IDS.has(sourceId)) {
    const already = new Set(cricketDocs.map((doc) => doc.channelId));
    for (const doc of docs) {
      if (already.has(doc.channelId)) continue;
      // v10.2.2: the strict allowlist (Indian sports + cricket-centric only)
      // — the old /(sport|cricket|willow|ppv)/ sweep mapped 500+ country channels.
      if (isWantedSportsChannel(toClientChannel(doc))) candidates.push(doc);
    }
  }
...
export async function ensureLiveServiceSeeded() {
  ...
  // Purge sources whose upstream died or was withdrawn (v10.2.0 sports packs
  // with dead segments; v10.3.0 the country-wide romaxa55 list). For a
  // REMOVED source the whole channel set goes (favorites spared) — keeping
  // rows of a source that no longer exists only leaves catalog ghosts.
  const allSources = await LiveSource.find({}).select('sourceId label url').lean();
  for (const doc of allSources) {
    if (!isRemovedSource(doc)) continue;
    await LiveSource.deleteOne({ sourceId: doc.sourceId }).catch(() => {});
  }
  // Keyed on explicit ids, NOT on surviving docs: releases before v10.3.1
  // deleted the source docs but left their mapped channels behind as orphans,
  // which is exactly why withdrawn sources kept haunting the Sports catalog.
  const gone = await LiveChannel.deleteMany({
    sourceId: { $in: REMOVED_SOURCE_IDS },
    favorite: { $ne: true },
  }).catch(() => null);
  if (gone?.deletedCount) await recalcSourceCounts().catch(() => {});

  // v10.2.2: the sports source was once auto-mapped country-wide (500+
  // channels in the Sports catalog). Demap every romaxa55 row the strict
  // allowlist does not want. Only rows from that source are touched —
  // curated mappings from the Jio/JSON sources are never disturbed.
  try {
    const flooded = await LiveChannel.find({
      sourceId: 'romaxa55-sports',
      'catalogs.0': { $exists: true },
      favorite: { $ne: true },
    }).select('channelId name category language catalogs').lean();
    let demapped = 0;
    for (const doc of flooded) {
      if (isWantedSportsChannel(toClientChannel(doc))) continue;
      await LiveChannel.updateOne(
        { channelId: doc.channelId },
        { $set: { catalogs: [], selected: false } },
      ).catch(() => {});
      demapped += 1;
    }
    if (demapped) await recalcSourceCounts('romaxa55-sports').catch(() => {});
  } catch { /* first boot without the source — nothing to clean */ }
