...
import { isRemovedSource } from '@/lib/liveTv';
...
async function autoMapNewSourceCricketChannels(sourceId) {
  ...
  if (SPORTS_CATALOG_SOURCE_IDS.has(sourceId)) {
    ...
    for (const doc of docs) {
      ...
      const client = toClientChannel(doc);
      if (/(sport|cricket|willow|ppv)/i.test(`${client.name || ''} ${client.category || ''}`)) candidates.push(doc);
    }
  }
