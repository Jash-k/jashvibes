...
export async function syncLiveSource(sourceDoc, { includeAll = true, autoSelectPopular = false } = {}) {
  ...
  for (const channel of parsed) {
    ...
    const shouldAutoSelect = Boolean(autoSelectPopular && /star vijay|sun tv|zee tamil|ktv|vijay super|adithya|thanthi|news18 tamil|polimer|puthiya|jaya tv/.test(key));
    // Do not put selected/favorite/hidden/order/profiles in both $set and
    // $setOnInsert. Mongo treats that as a path conflict and the whole sync
    // aborts, which made source cards stay at 0 channels.
    delete set.selected;
    delete set.favorite;
    delete set.hidden;
    delete set.order;
    ...
    if (existing && shouldAutoSelect && !existing.selected) {
      await LiveChannel.updateOne({ channelId: doc.channelId }, { $set: { selected: true } });
    }
    stored += 1;
  }
