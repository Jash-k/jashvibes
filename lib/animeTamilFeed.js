...
<script>
  (function () {
    ...
    window.open = function (url) { blocked += 1; report({ popup: String(url || '').slice(0, 220) }); return null; };
    document.addEventListener('click', function (event) {
      var node = event.target;
      while (node && node.tagName !== 'A') node = node.parentElement;
      if (!node) return;
      if (node.target === '_blank') { event.preventDefault(); blocked += 1; report({ popup: String(node.href || '').slice(0, 220) }); }
    }, true);
