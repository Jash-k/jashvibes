 ...
 *
 * Each row also carries `quality` (a short resolution token like "1080p", or
 * '') and `sizeBytes`/`size` when the upstream stream knows them — that is
 * what the player's quality pill and stream list print, so the pill and the
 * page's own dropdown describe the same stream with the same words.
 */
const RESOLUTION_RE = /\b(2160p|1440p|1080p|720p|576p|540p|480p|360p|240p)\b/i;

function resolutionFrom(text = '') {
  const hit = String(text || '').match(RESOLUTION_RE)?.[0];
  if (hit) return hit.toLowerCase();
  return /\b(?:4k|uhd)\b/i.test(String(text || '')) ? '4K' : '';
}
...
export function buildSourceList({ urls = [], streams = [], labelFor } = {}) {
  return (urls || [])
    ...
    .map((url, index) => {
      ...
      const label = metaLabel || custom || parseUrlSourceLabel(url) || `Source ${index + 1}`;
      const quality =
        (String(matched?.quality || '').match(/^(?:\d{3,4}p|4K)$/i)?.[0] || '').toLowerCase() ||
        resolutionFrom(matched?.label) ||
        resolutionFrom(metaLabel) ||
        resolutionFrom(custom) ||
        resolutionFrom(parseUrlSourceLabel(url));
      return {
        url,
        index,
        label,
