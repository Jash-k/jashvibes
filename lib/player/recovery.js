 ...
 * @param {object} caps  { canRetryStreaming, canReanchor, canReload, hasDrm, hasFallbackSources, hasPolicyRecovery, live, offline, errorKind, aborted, seeking }
...
export function nextRecoveryAction(state = {}, caps = {}, ladder = DEFAULT_LADDER) {
  ...
  // A seek into a part of the file that has not been fetched yet looks exactly
  // like a stall from here (no currentTime progress for seconds), but the only
  // thing that fixes it is waiting. Reloading mid-seek restarts a plain file
  // from 0 — the classic "seek jumps back to the beginning" complaint on
  // Telegram/Stremio direct links — so the ladder stays completely out of it.
  if (caps.seeking) return null;
  ...
  for (let i = lastIndex + 1; i < ladder.length; i += 1) {
    ...
    return {
      ...
      // Every rung carries the position: for a manifest the element keeps its
      // buffer, and for a plain file a reload without the position is how a
      // recovery silently turns into "watch it from the start again".
