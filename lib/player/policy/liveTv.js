...
export function createLiveTvPolicy(channel = {}, options = {}) {
  ...
  async function resolve({ force = false, prior = null } = {}) {
    ...
    const requestFilter = (requestType, request, ctx = {}) => {
      ...
      if (state.liveProxyEnabled && /^https?:\/\//i.test(nextUri)) {
    ...
    if (state.liveProxyEnabled && /^https?:\/\//i.test(playbackUrl)) {
      // After a direct failure the manifest starts on the proxy too; the response
      // filter restores the real URI so relative segments still resolve upstream,
      // and the request filter proxies each of those as well.
  ...
  return {
    ...
    hasRecovery: () => Boolean((pocket && !state.pocketProxyEnabled) || (usesJio && !state.jioProxyEnabled) || (liveProxy && !state.liveProxyEnabled)),
    describe: () => ({ ...state }),
  };
}
