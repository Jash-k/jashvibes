...
/**
 * The generic live proxy (`/api/live-proxy`) for header-carrying channels that are neither
 * Jio nor Pocket — the playlist-family streams (FanCode, SonyLiv, ICC dumps). Their edges
 * serve the manifest open but can 403 the variant/segment hosts, and browsers cannot set
 * User-Agent/Referer on cross-origin requests anyway, so when direct playback fails the
 * whole stream rides our own server, which can set anything.
 */
export function needsProxyFallback(channel = {}) {
  return !isJioChannel(channel) && !isPocketChannel(channel)
    && Boolean(String(channel.referer || '').trim() || String(channel.userAgent || '').trim() || String(channel.cookie || '').trim());
}

export function buildLiveProxyUrl(uri = '', channel = {}) {
  const params = new URLSearchParams({ u: uri });
  if (channel.userAgent) params.set('ua', channel.userAgent);
...
export function restoreLiveProxyUri(uri = '', origin = '') {
  try {
    const parsed = new URL(uri, origin || 'http://localhost');
    if (parsed.pathname === '/api/live-proxy') return parsed.searchParams.get('u') || uri;
  } catch {}
  return uri;
}

/**
 * The remote stream proxy: the owner's own Cloudflare Worker (docs/STREAM-WORKER.md,
 * free tier, one deploy). It egresses near the VIEWER, so a geo-fenced CDN (FanCode,
 * SonyLiv — India-only edges) answers it, it adds CORS to CORS-less CDNs, and it
 * rewrites playlists so every segment rides it too. This is the rung that actually
 * unlocks FanCode for a viewer in India; the server-side proxy egresses from wherever
 * the app is hosted and cannot follow a geo fence.
 */
export function buildStreamProxyUrl(uri = '', channel = {}) {
  const base = String(channel.streamProxy || '').trim().replace(/\/?$/, '');
  if (!base || !/^https?:\/\//i.test(uri)) return uri;
...
export function onStreamProxy(uri = '', channel = {}) {
  const base = String(channel.streamProxy || '').trim().replace(/\/?$/, '');
  return Boolean(base) && String(uri || '').startsWith(`${base}/`);
}
...
export function createLiveTvPolicy(channel = {}, options = {}) {
  ...
  const liveProxy = needsProxyFallback(channel);
  const streamProxy = Boolean(String(channel.streamProxy || '').trim());
  ...
  async function resolve({ force = false, prior = null } = {}) {
    ...
    const requestFilter = (requestType, request, ctx = {}) => {
      ...
      /* A URI that is already on a proxy stays exactly as it is: the proxy rewrote
         the playlist so children arrive pre-proxied, and wrapping again would
         double-encode them. */
      const alreadyProxied = onStreamProxy(originalUri, channel) || /\/api\/live-proxy(\?|$)/.test(originalUri);
      ...
      if (alreadyProxied) {
        // nothing to add — the proxy's playlist rewrite owns these from here
      } else if (state.streamProxyEnabled && /^https?:\/\//i.test(nextUri)) {
        request.uris[0] = buildStreamProxyUrl(nextUri, channel);
        delete request.headers['User-Agent'];
        delete request.headers.Referer;
        delete request.headers.Cookie;
      } else if (state.liveProxyEnabled && /^https?:\/\//i.test(nextUri)) {
        // Direct playback already failed once: everything rides the server now,
        // which can set the User-Agent/Referer browsers are not allowed to send.
        request.uris[0] = buildLiveProxyUrl(nextUri, channel);
        delete request.headers['User-Agent'];
        delete request.headers.Referer;
        delete request.headers.Cookie;
      } else if (state.pocketProxyEnabled && /^https?:\/\//i.test(nextUri)) {
    ...
    const responseFilter = (requestType, response) => {
      ...
      if (state.liveProxyEnabled) response.uri = restoreLiveProxyUri(response.uri, origin);
    ...
    const needsHeaderRewrite =
      ...
      state.liveProxyEnabled ||
      state.streamProxyEnabled ||
    ...
    // The most recently enabled transport wins: the rungs enable in order
    // (stream proxy, then the render proxy), so check the later one first.
    if (state.liveProxyEnabled && /^https?:\/\//i.test(playbackUrl)) {
      // After a direct failure the manifest starts on the proxy too; the proxy
      // rewrites the playlist so every child rides it as well.
      resolvedUrl = buildLiveProxyUrl(playbackUrl, channel);
    } else if (state.streamProxyEnabled && /^https?:\/\//i.test(playbackUrl)) {
      resolvedUrl = buildStreamProxyUrl(playbackUrl, channel);
    }
  ...
  async function recover({ error = null } = {}) {
    ...
    if (streamProxy && !state.streamProxyEnabled) {
      state.streamProxyEnabled = true;
      return { message: 'Direct playback failed — retrying through your stream proxy (viewer-region exit, CORS open)…', retry: 'reload' };
    }

    if (liveProxy && !state.liveProxyEnabled) {
      state.liveProxyEnabled = true;
      return { message: 'Direct playback failed — retrying through this server\u2019s live proxy with the stream\u2019s own headers…', retry: 'reload' };
    }
  ...
  return {
    ...
    hasRecovery: () => Boolean((pocket && !state.pocketProxyEnabled) || (usesJio && !state.jioProxyEnabled) || (streamProxy && !state.streamProxyEnabled) || (liveProxy && !state.liveProxyEnabled)),
    describe: () => ({ ...state }),
  };
}
