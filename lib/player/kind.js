...
/** Highest end time across every buffered range (0 when nothing is buffered). */
export function readBufferedEnd(el) {
  try {
    const buffered = el?.buffered;
    if (!buffered || !buffered.length) return 0;
    let end = 0;
    for (let i = 0; i < buffered.length; i += 1) end = Math.max(end, Number(buffered.end(i)) || 0);
    return end;
  } catch {
    return 0;
  }
...
/**
 * Where a seek may legally land.
 *
 * By default this is `clampToSeekWindow`: a finite `duration` wins, because for a plain file
 * `seekable` only reports what has downloaded so far and treating that as the legal range silently
 * rewrote "jump to 58:00" into "jump to 4s".
 *
 * `coarse` is the exception, and it is earned rather than assumed: once a seek has been *proved* to be
 * refused (asked for X, landed nowhere near X, twice) the browser cannot resolve a jump in this file.
 * Two causes produce identical symptoms and both are outside our control: the server does not answer
 * byte-range requests, or the container has no usable seek index for a browser demuxer (an MKV/WebM
 * without `Cues`, which Chromium answers with a linear forward scan — i.e. it re-reads the file, which
 * is what looks like "the seek restarted at 0"). From then on the reachable range really is what has
 * been read, so a further drag is clamped to it instead of being traded for a reload at 0.
 */
export function clampSeekTarget(el, target, { coarse = false, lead = 5 } = {}) {
  const win = readSeekWindow(el);
  const value = Number(target);
  if (!win || !Number.isFinite(value)) return null;
  const from = win.start;
  if (!coarse) return Math.min(Math.max(from, value), Math.max(from, win.end - 0.25));
  const current = Number(el?.currentTime) || from;
  const reachable = Math.max(current + 1, readBufferedEnd(el) - Math.max(0.5, lead));
  const ceiling = Math.min(Math.max(from, win.end - 0.25), Math.max(from, reachable));
  return Math.min(Math.max(from, value), ceiling);
}
