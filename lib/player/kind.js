 ...
 *
 * `noRange` is the exception, and it is earned rather than assumed: once a seek has been *proved* to
 * be refused (asked for X, landed nowhere near X, twice), the host does not answer byte-range requests.
 * From then on the reachable range really is only what has downloaded, so a further drag is clamped
 * to it — which is what stops the player from being talked into a reload that restarts the file at 0.
 */
export function clampSeekTarget(el, target, { noRange = false, lead = 5 } = {}) {
  const win = readSeekWindow(el);
  const value = Number(target);
  if (!win || !Number.isFinite(value)) return null;
  const from = win.start;
  if (!noRange) return Math.min(Math.max(from, value), Math.max(from, win.end - 0.25));
