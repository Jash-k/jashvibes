...
async function fetchSpotifyMetadataViaProxy(url = '') {
  ...
  try {
    ...
    if (error.name === 'AbortError') throw new Error('Spotify proxy timed out while reading the playlist. Try again.');
    throw error;
  } finally {
...
async function matchSpotifyTrackToSaavn(spotifyTrack = {}) {
  ...
  const lists = await Promise.all([
    searchSongs(query, 8).catch(() => []),
    fallbackQuery && fallbackQuery !== query ? searchSongs(fallbackQuery, 6).catch(() => []) : [],
  ]);
...
export async function importSpotifyPlaylist(url = {}, options = {}) {
  ...
  const maxTracks = Math.max(1, Math.min(Number(options.maxTracks || process.env.SPOTIFY_IMPORT_MAX_TRACKS || 120), 300));
