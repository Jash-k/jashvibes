...
export function groupFeed(items = [], { now = Date.now(), pin = '' } = {}) {
  ...
  const groups = [
    { id: 'live', label: 'Live now', newestLast: false, items: items.filter((item) => item.state === 'live' && !item.staleFeed) },
    ...
    { id: 'unverified', label: 'Unverified — feed too old to trust', newestLast: false, items: items.filter((item) => item.state === 'live' && item.staleFeed) },
  ];
  ...
  return groups
    ...
    .map((group) => {
      const cap = group.id === 'done' || group.id === 'unverified' ? 8 : 24;
