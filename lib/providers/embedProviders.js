...
export function buildOmegaEmbed({ tmdbId, imdbId = '', type, season = 1, episode = 1 }) {
  const embedType = normalizeType(type);
  // Omega follows the same behavior as the Sanjay watch page: prefer IMDb when
  // available, then fall back to TMDB. Some newer titles (example DC / tmdb
  // 1479832) only resolve correctly on Omega with the IMDb id.
  const id = String(imdbId || tmdbId || '').trim();
