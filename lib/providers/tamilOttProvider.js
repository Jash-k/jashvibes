...
function mapFeedItem(item, feedUrl = '') {
  ...
  const title = String(omdb.Title || cleanTitle || '').trim();
  const type = String(omdb.Type || '').toLowerCase() === 'series' ? 'series' : 'movie';
...
async function loadTamilOttItems() {
  ...
  const feedResults = await Promise.allSettled(urls.map((url) => fetchJsonFeed(url)));
  const feeds = feedResults
    .filter((result) => result.status === 'fulfilled')
    .map((result) => result.value);

  if (!feeds.length) {
    const reason = feedResults.find((result) => result.status === 'rejected')?.reason?.message || 'All configured TamilOTT JSON feeds failed';
  ...
  const items = feeds
    ...
    .filter((item) => {
      const key = item.streamUrl || `${item.feedUrl}:${item.streamId}`;
...
export async function resolveTamilOttProvider({ tmdbId, type, title = '', year = '', season = 1, episode = 1, streamId = '' }) {
  const items = await loadTamilOttItems();
  const meta = await getTMDBMeta({ tmdbId, type, title, year });
  ...
  if (!candidates.length) {
    throw new Error(`TamilOTT JSON has no matching authorized stream for ${meta.title || tmdbId}.`);
  }
  ...
  if (requestedId) {
    ...
    if (exactRequestedItem && !candidates.some((entry) => entry.item.streamId === exactRequestedItem.streamId)) {
      candidates.unshift({ item: exactRequestedItem, baseScore: 9999 });
    }

    selectedEntry = candidates.find((entry) => String(entry.item.streamId) === requestedId || String(entry.item.id) === requestedId) || null;
    if (!selectedEntry) {
      throw new Error('Selected TamilOTT stream is no longer available for this title.');
    }
  }
...
function toCatalogItem(item, source = 'movies') {
  return {
    id: `ott-${source}-${item.streamId}`,
    streamId: item.streamId,
    type: item.type || 'movie',
    title: item.title || item.cleanStreamTitle || 'Untitled',
    streamTitle: item.streamTitle || '',
    category: source === 'dubbed' ? 'Dubbed' : item.source || 'TamilOTT',
    year: item.year || '',
    season: item.season,
    episode: item.episode,
    releaseDate: item.year ? `${item.year}-01-01` : '',
    posterUrl: item.posterUrl || '',
    synopsis: item.synopsis || item.streamTitle || '',
    rating: item.rating || '',
    quality: item.quality || '',
    size: item.size || '',
...
export async function listTamilOttCatalog({ source = 'movies', page = 1, limit = 15, query = '' } = {}) {
  ...
  const rawItems = await fetchJsonFeed(selectedUrl);
  const q = normalizeTitle(query);
  const grouped = new Map();

  for (const item of rawItems) {
    if (!item.streamUrl || !item.title) continue;
    if (q) {
    ...
    // LIFO: highest/latest stream id wins for the visible catalogue card.
    if (!existing || Number(item.id || 0) > Number(existing.id || 0)) {
      grouped.set(key, item);
    }
  }

  const sorted = [...grouped.values()].sort((a, b) => Number(b.id || 0) - Number(a.id || 0));
  const safeLimit = Math.max(1, Math.min(60, Number(limit) || 15));
  const safePage = Math.max(1, Number(page) || 1);
  const start = (safePage - 1) * safeLimit;
  const pageItems = sorted.slice(start, start + safeLimit).map((item) => toCatalogItem(item, selectedSource));
