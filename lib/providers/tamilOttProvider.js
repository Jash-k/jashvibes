...
async function loadTamilOttItemsForResolve({ quick = true, manual = false, streamId = '', scanLimit = 0 } = {}) {
  ...
  let exactRequestedItem = null;
  if (streamId) {
    exactRequestedItem = await findTamilOttItemByStreamId(streamId, maxObjects);
  }
  ...
  if (exactRequestedItem?.streamUrl) {
    const exactKey = exactRequestedItem.streamUrl || `${exactRequestedItem.feedUrl}:${exactRequestedItem.streamId}`;
    if (!seen.has(exactKey)) items.unshift(exactRequestedItem);
  }
...
function isPreferredOttQuality(item = {}) {
  const text = `${item.quality || ''} ${item.displayLabel || ''} ${item.streamTitle || ''} ${item.title || ''}`;
  return /\b(1080p|720p)\b/i.test(text);
}
...
export async function resolveTamilOttProvider({ tmdbId, type, title = '', year = '', season = 1, episode = 1, streamId = '', quick = false, scanLimit = 0 } = {}) {
  ...
  const visibleCandidates = meta.type === 'movie'
    ? candidates.filter((entry) => isPreferredOttQuality(entry.item))
    : candidates;
  const dropdownCandidates = visibleCandidates.length ? visibleCandidates : candidates;
  const availableStreams = dropdownCandidates.map((entry) => toStreamOption(entry.item, finalScore(entry.item, meta, { season, episode })));
  const fallbacks = dropdownCandidates
...
export async function listTamilOttCatalog({ source = 'movies', page = 1, limit = 15, query = '' } = {}) {
  ...
  for (const rawItem of rawItems) {
    ...
    const existingPreferred = isPreferredOttQuality(existing.item || {});
    const itemPreferred = isPreferredOttQuality(item);
    // Prefer clean 1080p/720p entries for catalogue cards/dropdowns. If both
    // entries are same preference class, latest stream id wins.
    if (
      !existing.item ||
      (itemPreferred && !existingPreferred) ||
      (itemPreferred === existingPreferred && Number(item.id || 0) > Number(existing.latestId || 0))
    ) {
      existing.item = item;
