...
async function fetchJsonFeedHeadCached(url, maxObjects = 240) {
  ...
  if (full?.items && Date.now() - full.loadedAt < CACHE_TTL_MS) return full.items.slice(0, maxObjects);

  const cached = cache.catalogHead.get(url);
  if (cached?.items && cached.maxObjects >= maxObjects && Date.now() - cached.loadedAt < CACHE_TTL_MS) {
