...
export function getVodSources() {
  ...
  const multi = String(process.env.VOD || process.env.VOD_M3U || process.env.CLASSICS || '').trim();
  if (multi) {
    multi
      .split(',')
      ...
      .filter(Boolean)
      .forEach((entry, index) => {
        const parsed = parseSourceEntry(entry, index);
        if (parsed) sources.push(parsed);
      });
  }

  const eros = normalizeHttpUrl(process.env.VOD_EROS || process.env.EROS_M3U || '');
  if (eros) sources.push({ label: 'ErosNow', url: eros });

  const aha = normalizeHttpUrl(process.env.VOD_AHA || process.env.AHA_M3U || '');
  if (aha) sources.push({ label: 'Aha', url: aha });
...
async function fetchText(url) {
  ...
  if (!response.ok) throw new Error(`VOD source returned HTTP ${response.status}`);
