...
import { humaniseSlugText, looksLikeHumanTitle, slugToTitle } from '@/lib/releaseTitle';

const DEFAULT_BASE_URL = 'https://www.1tamilmv.lease/';
const USER_AGENT =
...
function titleCaseSlug(slug = '') {
  // Release slugs are dot-separated and often camel-humped (`ImmortalCombat.2025.[Tamil].1080p`), so
  // splitting only on '-' produced one glued word — which then also failed the TMDB lookup, because
  // that normaliser can drop punctuation but cannot invent a space. See lib/releaseTitle.js.
...
/**
 * The one truth on this forum: the topic URL slug carries the real release
 * name (`/topic/199944-ramba-oorvasi-menaka-2026-tamil-true-web-dl-…`). The
 * new markup splits anchor text so some links carry only the quality bracket;
 * when the parsed title is junk, the slug words BEFORE the year/language/
 * quality tokens are the title.
 */
function titleFromTopicSlug(url = '') {
  const slug = extractSlugText(url);
  if (!slug) return '';
  const kept = [];
  for (const word of slug.split(' ')) {
    const clean = word.toLowerCase().replace(/[^a-z0-9.]/g, '');
    if (!clean) continue;
    if (/^(19|20)\d{2}/.test(clean)) break;                       // the year ends the title here
    if (/^(?:tamil|telugu|malayalam|hindi|kannada|english|tam|tel|mal|hin|kan|eng|multi|dual)$/.test(clean)) break;
    if (/^(?:true|hq|org|original|web|dl|webdl|hdrip|hdtv|predvd|dvdscr|bluray|blu|ray|bdrip|brrip|x264|x265|h264|h265|hevc|avc|aac|ac3|dd|dd5|dd51|10bit|8bit|hdr|hdr10|dolby|vision|amzn|nf|atvp|hulu|esub|esubs|subs|untouched|4k|2160p|1080p|720p|480p|360p|s\d{1,2}|ep\d{1,3})$/.test(clean)) break;
    if (/^\d+(kbps|mb|gb|p)$/.test(clean) || /^[0-9.]+$/.test(clean)) break;
    kept.push(word);
  }
...
/** Titles the parser can be handed that are markup noise, never a movie. */
function looksLikeJunkTitle(title = '') {
  const value = String(title || '').trim();
  if (value.length < 2) return true;
  if (/^[\[\/]/.test(value)) return true;
  if (/index\s*php|forums?\s*topic|topic\s*\d+/i.test(value)) return true;
  const letters = value.replace(/[^A-Za-z]/g, '');
  return letters.length < Math.max(2, Math.floor(value.length * 0.4));
}
...
function parseReleaseMetadata(text = '', url = '') {
  ...
  // Used to be "visible text, but only if somebody typed a year" — which threw away a perfectly good
  // title like `Immortal (G.V. Prakash)` and preferred the release slug instead. Prefer the human
  // words whenever there are any; fall back to the slug only when the anchor is pure quality noise.
  let source = looksLikeHumanTitle(visible) ? visible : slugText || visible || text;
...
export async function scrapeTamilMV({ fetchPostersEnabled = false, maxPosterFetches = 8, limitPerType = 15, matchTMDB = true, includeTvShows = true } = {}) {
  ...
  $('a').each((_, el) => {
    ...
    const decodedHrefText = humaniseSlugText(decodeURIComponent(hrefPath));
    ...
    let parsedSource = parsed.parsedSource;
    // The homepage markup sometimes leaves an anchor holding only the quality
    // bracket; the parser then "titles" the row `"[4K, 1080p & 720p"` or a URL
    // fragment. Repair from the topic slug, or refuse to publish the row.
    if (looksLikeJunkTitle(title)) {
      const slugTitle = titleFromTopicSlug(threadUrl);
      if (!slugTitle) return;
      title = slugTitle;
      parsedSource = 'topic-slug';
    }
    ...
    const item = {
      ...
      qualityTier: parseReleaseQuality(releaseText).tier,
      qualityLabel: parseReleaseQuality(releaseText).label,
      isTopRelease,
