...
2. `/series/<slug>` → seasons from `a.season-btn` and every episode from `ul#episode_by_temp` (`span.num-epi` gives `1x3`, the `w185` still is the thumbnail). A `/movies/<slug>` page has no episode list, so it is given one row — a film is still playable from the same sheet.
3. `/episode/<slug>` → `div#options-N` rows, `data-src` for lazy ones and `src` for the active one, deduped, with `type` = `wrapped` / `multi` / `rotator` / `embed`.
4. the host page → a stream. `HOST_RULES` is the whole allow-list: **Vidmoly** (`sources:[{file:'…master.m3u8?…'}]` in plain text) and **TurboVid**. Anything else is reported, not fetched.

**Why only those hosts.** Measured on a real episode: Vidmoly's playlist *and* its `.ts` segments answer `access-control-allow-origin: *` with no referer, no cookie and a 12 h token, so `hls.js`/Shaka in the browser plays it — 1280×720 here, `readyState 4`, clock advancing. TurboVid's master resolves too (480/720/1080) but its segments are `lh3.googleusercontent.com/d/<id>=d`, which a page cannot fetch, so `SOURCE_PRIORITY` puts Vidmoly first and TurboVid stays *offered*. The site's own `proxy/play.php?url=https://as-cdn26.top/video/<hash>` is a JWPlayer page whose address is assembled by obfuscated script and ad overlays: refused with the reason, next to a link to the source. Its per-language map (`proxy/multi.php?data=<base64>` → Hindi/Tamil/Telugu/English/Japanese) points at `short.icu`, which is NXDOMAIN at the registry, so the row says `"lists Hindi, Tamil, … on a shortener that no longer resolves"` and nothing is fetched from it.

**The catalogue is the whole taxonomy.** 24 titles a page, 13 pages, **298 titles — 195 series and 103 films**, counted in the browser after `Read all 13 pages`. The walk does not trust the pagination widget in either direction: it runs to `MAX_LISTING_PAGE` and stops at the first page whose list grid is empty, and reports the pages it actually read (`all 13 pages` when it ended on an empty page, `first N pages` when it ran out of budget). An earlier version stopped at the widget's hint and quietly skipped page 14's rows; the fixture keeps all three grids so the exclusion is tested, not asserted.

**Nothing is a decoration.** `Movies`/`Series` filter what has loaded (24 → 3 → 21 → 24, counted in the browser, and the same chips over a full walk read 103 and 195); `Search` runs the source's own `?s=` through the route and says so in the mast (`· search “naruto”`); `Next page` and `Read all 13 pages` grow the grid; `Reload` re-reads past the cache; a deep link `/anime/tamil?t=/series/<slug>` reopens the sheet; `Escape`, the scrim and the close path all dismiss it; a row that did not resolve is `disabled` and shows its reason ("the address is built by that host's script, so it is not playable here", "its rotator offers no resolvable host"), and every row the source listed is shown, not only the ones that work; the playable rows are the player's own source menu (`lineup`), and a stream that dies mid-play walks to the next resolved address (`stepToNextSource`, one attempt each) before saying "every address this episode offered was tried".

Resume is `library={{ watchKey, entry }}` — the entry matters, because a `watchKey` without one lands in Continue Watching titled `Untitled`: the row carries `Clevatess` / `S1 · E1` / the poster, and the URL the player writes is `/anime/tamil?t=/series/<slug>&ep=/episode/<slug>-1x1/`, so the link opens the sheet *and* starts the episode it was left on (verified cold: `readyState 4`, clock advancing). One host note is printed under the player when the playing row is a manifest-without-segments host, and a retry after a failure re-resolves (`force=1`) instead of replaying the same expired token.

**Free-tier shape, same rules as the sports board.** `globalThis.__jashAnimeTamil`, single-flight per key, 6 h for the list and titles, 30 min per episode, 30 s for a failure *including a page that answered with nothing*; `payload.ttlMs` is stamped with the window actually used so `Cache-Control` and the cache cannot disagree; only same-origin paths are fetchable at all (`openPath` refuses `/series/…` on another host and a traversal, and the routes take the path back rather than a URL); ad/tracker hosts on those pages are deny-listed so a rotator hop can only land on a host with a rule; no cron, no queue, no Mongo write, nothing on a timer. `/api/anime/tamil/play` is capped at 30/min and `/api/anime/tamil` at 60/min in `middleware.js`.
