...
/sports                   <JashPlayer source={…} playbackPolicy={liveTv policy} live />
...
- **Four controls, one tap each**: play, ±10 (hidden on phones — double-tap seeks
  there), time, volume (desktop) on the left; **Quality** (the active stream's
  resolution, e.g. "1080p"), **Aspect** (the current picture mode), **CC** (only
  when the stream has subtitle tracks) and **PiP** (only where the browser allows)
  on the right, then fullscreen. Each pill opens its own short list — no sheet,
  no second tap. A control the phone cannot see is a control that does not exist,
  so nothing here hides behind a `sm:` breakpoint; the ±10s buttons are the one
  deliberate exception, because a phone's double-tap already seeks.
...
## 6b. The quick menus (no settings sheet)

The all-in-one settings sheet is deleted. Quality and Aspect each open their own
list straight from the bar; CC and PiP are plain toggles. `QualityMenu` prints
**the stream list the page discovered** when there is one (Stremio files with
sizes, mirrors), the engine's Auto + rendition heights when there is not, and a
sentence when the source exposes a single rendition. Picking a stream calls
`engine.rotateFallback(index)` and `lineup.onPickSource(index)` in one motion —
the engine switches with the position kept and the page's own dropdown re-syncs
(the Watch page derives its Stremio select's value from the URL actually playing,
so auto-rotation past a dead stream can no longer leave it stale).

Everything the sheet used to hold still *works*, just without UI: speed,
audio/subtitle options and the shortcuts list are keyboard commands (see §5's
command map), freeze/stats/restart/next-source live in the right-click context
menu, and `openSubtitles` (shift+C) now simply toggles captions since its sheet
is gone. Command metadata declares only controls that exist — the parity test
fails the build on a stale claim.
...
`/live`, `/watch`, `/classics`, `/sports` and `/stremio-watch` all hand the chrome `'fill'` to say
