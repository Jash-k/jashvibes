...
test('the choice is a viewer preference with a do-nothing default', () => {
  ...
  assert.match(player, /<AspectMenu[\s\S]*?onPick=\{\(value\) => \{[\s\S]*?engine\.setPref\('aspect', value\)/,
    'picking persists through the prefs hook');
});

test('the picture choice is its own one-tap menu, not a section of a sheet', () => {
  const menus = read('../components/player/PlayerMenus.js');
  assert.match(menus, /export const AspectMenu/, 'the aspect list is a standalone menu now');
  assert.match(menus, /ASPECT_MODES\.map\(\(mode\) => \(/);
  assert.match(menus, /active=\{aspect === mode\.id\}/, 'the active mode has to be legible at a glance');
  assert.match(menus, /hint=\{mode\.hint\}/, 'every mode carries its hint (letterbox / crop bars / scope…)');
  assert.ok(!/MenuSection label="Aspect ratio"/.test(menus), 'the settings sheet it used to live in is gone');
});
